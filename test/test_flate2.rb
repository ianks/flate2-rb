...
class TestFlate2 < Minitest::Test
  ...
  def test_gzip_roundtrip_small_string
    assert_equal("hello", Zlib.gunzip(Flate2.gzip("hello")))
  end

  def test_gzip_roundtrip_large_string
    content = SecureRandom.random_bytes(1024 * 1024 * 8)

    assert_equal(content, Zlib.gunzip(Flate2.gzip(content)))
  end
  ...
  def test_gzip_treats_string_as_binary
    ...
    assert_equal("hello".encode(Encoding::UTF_16LE), Zlib.gunzip(result).force_encoding(Encoding::UTF_16LE))
  end

  def test_gunzip_treats_string_as_binary
    result = Flate2.gunzip(Zlib.gzip("hello".encode(Encoding::UTF_16LE)))

    refute_equal("hello", result)
    assert_equal("hello".encode(Encoding::UTF_16LE), result.force_encoding(Encoding::UTF_16LE))
  end
